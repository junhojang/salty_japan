class TestValidator
  ...
  def self.chk_level_test_available(params)
    ...
    if !params[:user_id].present?
      ...
      return false,MsgMaker.make_msg(MsgMaker::TYPE_FAILED,'chk_level_test_available',MsgMaker::NOT_EXIST,'user'),nil
  ...
  def self.set_log_test(params)
    if !params[:test_type].present? or !params[:user_id].present?
